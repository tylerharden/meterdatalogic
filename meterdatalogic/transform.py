...
def resample_energy(df: pd.DataFrame, rule: Literal["15min","30min","1H","1D","1MS"]) -> pd.DataFrame:
    # Sum energy while preserving keys using pd.Grouper (future-proof vs GroupBy.resample)
    ...
    # keep cadence_min: recompute from the new index (unchanged cadence for coarser resamples)
    out["cadence_min"] = (60 if rule == "1H" else 1440 if rule in ("1D","1MS") else int(rule.replace("min","")))
    return out
...
def profile24(df: pd.DataFrame, by: str = "flow", agg: Literal["mean","median"]="mean") -> pd.DataFrame:
    # average day shape per interval slot
    s = df.copy()
    s["slot"] = s.index.strftime("%H:%M")
    g = s.groupby(["slot", by])["kwh"]
    out = (g.mean() if agg == "mean" else g.median()).unstack(by).fillna(0.0).reset_index()
    return out
...
def tou_bins(df: pd.DataFrame, bands: Iterable[dict]) -> pd.DataFrame:
    ...
    for band in bands:
        start = _parse_time_str(band["start"])
        end = _parse_time_str(band["end"])
        mask = _time_in_range(times, start, end)
...
def demand_window(df: pd.DataFrame, start: str="16:00", end: str="21:00", days: str="MF") -> pd.DataFrame:
    s = df.copy()
    ...
    timemask = _time_in_range(t.time, start_t, end_t)
